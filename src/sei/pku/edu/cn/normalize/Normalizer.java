package sei.pku.edu.cn.normalize;
...
import sei.pku.edu.cn.normalize.common.tools.ErrorManager;
import sei.pku.edu.cn.normalize.common.util.Pair;
import sei.pku.edu.cn.normalize.common.util.VariableGenerator;
import sei.pku.edu.cn.normalize.phases.Generator;
import sei.pku.edu.cn.normalize.phases.WrappedStatement;
...
public class Normalizer {
	...
	public static WrappedStatement normalizeStmt(Statement input) {
		...
		if (input instanceof AssertStatement) {
			...
			System.out.println(wrappedExp.getFirst().toString());
			
			return wrapStatement(wrappedExp.getFirst());
			
		} else if (input instanceof Block) {
			
			System.out.println("Normaliz‘er >> method >> normalizeStmt >> @29 Statement(Block):"+input);
			...
			System.out.println(expPair.getFirst().toString());
			
			return wrapStatement(expPair.getFirst());
		
		} else if (input instanceof LabeledStatement) {
			
			System.out.println("Normalizer >> method >> normalizeStmt >> @1235 Statement(LabeledStatement):"+input);
			...
			System.out.println("Normalizer >> method >> normalizeStmt >> @249 Statement(ReturnStatement):"+input);
			...
			if (node.getExpression() != null) {
				...
				System.out.println(expPair.getFirst());
...
//			Pair<List<Statement>, Expression> expPair = wrapExpression(normalizeExp(node.getExpression()));
	...
	private static Pair<List<Statement>,Expression> normalizeExp(Expression exp) {
		if (exp instanceof ArrayAccess) {
			...
			System.out.println(wrappedTarget.toString());
			...
			System.out.println(lhsPair.getFirst().toString());
